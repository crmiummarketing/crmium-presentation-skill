...
2. **Який варіант компонента**:
   - **`.qr-card`** (з підложкою) — коли поряд інший контент (заголовок, текст, картинка) і QR має бути в окремому "острівці".
   - **`.qr-card .qr-card--bare`** — коли QR є основним візуальним елементом слайда поруч з watermark або великим текстом (як на Resources-слайді або Q&A).

3. **Перевір фон слайда** (правило Принцип #0):
   - Темний фон → будь-який варіант працює (QR PNG є білий на прозорому)
   - Світлий фон → **ОБОВʼЯЗКОВО** `.qr-card` без `--bare`, з темним `background: var(--c-card)`

4. **Структура HTML** для додавання QR на ЛЮБИЙ слайд:
   ```html
   <!-- варіант з підложкою -->
   ...
   <!-- варіант без підложки -->
   <div class="qr-card qr-card--bare reveal">
...
0. **QR-коди працюють ТІЛЬКИ на темному фоні**. PNG-файли (`crmium-ask-question-qr.png`, `crmium-ua-bot-qr.png`) — білі pixel-и + помаранчеві куточки на прозорому фоні. На світлому фоні QR зникне. Якщо колись робиш слайд зі світлим фоном — **обовʼязково** клади QR на темну підложку (`qr-card` з `background: var(--c-card)`), не використовуй `qr-card--bare`. У дефолтних темних слайдах — будь-який варіант працює.
