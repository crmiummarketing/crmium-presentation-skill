...
### ⚠️ Якщо скіл не активувався (не задав питання)

Іноді Claude Code не "побачить" скіл по тригер-словах — особливо якщо повідомлення коротке або багатозначне. У такому разі **примусово виклич скіл через `/`-команду**:

```
/crmium-presentation-bento

Зроби мені презентацію про впровадження Zoho для виробничої компанії
```
...
Якщо `/crmium-presentation-bento` не зʼявляється у списку слеш-команд:
1. Перезапусти Claude Code (закрий повністю і відкрий заново).
2. Перевір що папка існує: `ls ~/.claude/skills/crmium-presentation-bento/` має повернути `SKILL.md`, `README.md`, `reference/`.
3. Якщо все ОК, але не активується — попроси Claude **явно прочитати скіл**:
   ```
   Прочитай скіл ~/.claude/skills/crmium-presentation-bento/SKILL.md і виконай його workflow для створення презентації.
   ```
